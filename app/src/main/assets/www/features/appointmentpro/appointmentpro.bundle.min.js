...
$scope.$on("$ionicView.beforeEnter",function(event,data){$scope.settings=Appointmentpro.getAppointmentproSettings();if($scope.settings.enable_location){$scope.getLocation()}else{$scope.loadContent()}});$scope.loadManagerBooking=function(){$scope.loadContent()}
...
console.log('location_info',$scope.location_info);$state.go("appointmentpro-category",{value_id:$scope.value_id,location_id:$scope.location_info.location_id},{reload:!0})};$scope.bookAService=function(service){if(!Customer.isLoggedIn()){$scope.login();return!1}
...
$scope.descriptionInfo=function(description){Dialog.alert($translate.instant("Description","appointmentpro"),description,"OK",60000,"appointmentpro")}
...
$scope.is_loading=!1}).error(function(error){$scope.is_loading=!1;Dialog.alert($translate.instant("Error","appointmentpro"),error.message,"OK",-1,"appointmentpro")}).finally(function(){$scope.$broadcast('scroll.infiniteScrollComplete')})};$scope.loadContent();$scope.goToBookingDetails=function(){$state.go("home",{value_id:$scope.value_id},{reload:!0}).then(function(){$state.go("appointmentpro-home",{value_id:$scope.value_id},{reload:!0}).then(function(){Loader.hide();$state.go("appointmentpro-booking",{value_id:$scope.value_id,type:'upcoming',service_type:$scope.service_type,booking_id:$scope.payout.booking.appointment_id},{reload:!0})})})}});angular.module('starter').controller('AppointmentproAccountController',function(Dialog,Loader,Application,$ionicSideMenuDelegate,Customer,$session,Modal,$ionicScrollDelegate,$timeout,$ionicModal,SB,$translate,$rootScope,$ionicLoading,$window,Appointmentpro,$scope,$state,$stateParams,$controller,$compile,$ionicPopup){angular.extend(this,$controller('AppointmentproCommanController',{Dialog:Dialog,$rootScope:$rootScope,$scope:$scope,$stateParams:$stateParams,Appointmentpro:Appointmentpro}));$scope.value_id=Appointmentpro.value_id=$stateParams.value_id;$scope.settings=Appointmentpro.getAppointmentproSettings();$scope.payout={};$scope.is_logged_in=Customer.isLoggedIn();$scope.customer=Customer.customer;$scope.notification_setting={};var callbacks={loaderror:logEvent,loadstart:logEvent,loadstop:logEvent,exit:logEvent};$scope.activeGoogle=function(){if(!$scope.settings.enable_google_calendar)return!1;var message=$translate.instant('Activate Calendar syncing?',"appointmentpro");if($scope.settings.providerInfo.google_refresh_token!=null){message=$translate.instant('Reactivate Calendar syncing?',"appointmentpro")}
$ionicPopup.show({title:$translate.instant('Confirm',"appointmentpro"),template:message,cssClass:'cancel-status',scope:$scope,buttons:[{text:$translate.instant('Cancel',"appointmentpro"),type:'button-default',onTap:function(e){return!1}},{text:$translate.instant('Yes',"appointmentpro"),type:'button-positive',onTap:function(e){return!0}}]}).then(function(result){if(result){var CLIENT_REDIRECT_URL=$scope.settings.google_redirect_URL;var CLIENT_ID=$scope.settings.client_id;var state=$scope.value_id+','+$scope.settings.providerInfo.providerId+','+device.platform;var googleURL='https://accounts.google.com/o/oauth2/auth?scope='+encodeURI('https://www.googleapis.com/auth/calendar')+'&redirect_uri='+encodeURI(CLIENT_REDIRECT_URL)+'&response_type=code&client_id='+CLIENT_ID+'&access_type=offline&include_granted_scopes=false&prompt=consent&state='+state;if(!Application.is_webview){$ionicPopup.show({title:$translate.instant('Info',"appointmentpro"),template:$translate.instant('Redirect to an absolute URL outside of the App',"appointmentpro"),cssClass:'cancel-status',scope:$scope,buttons:[{text:$translate.instant('Cancel',"appointmentpro"),type:'button-default',onTap:function(e){return!1}},{text:$translate.instant('Continue',"appointmentpro"),type:'button-positive',onTap:function(e){return!0}}]}).then(function(result){if(result){var strWindowFeatures="location=yes,scrollbars=yes,clearsessioncache=yes,clearcache=yes";var browser=$window.open(googleURL,'_system',strWindowFeatures,callbacks);browser.addEventListener('loadstart',function(event){if((event.url).indexOf(redirect_uri)===0){browser.removeEventListener("exit",function(event){});browser.close();var url=event.url;var first_split=url.split('?');var second_split=first_split[1].split('&');var token_split=second_split[0].split('=');var code=token_split[1];$scope.param={provider_id:$scope.settings.providerInfo.providerId,code:code};Appointmentpro.saveProviderGoogleToken($scope.param).then(function(data){$state.go("home",{value_id:$scope.value_id},{reload:!0}).then(function(){$state.go("appointmentpro-home",{value_id:$scope.value_id},{reload:!0}).then(function(){Dialog.alert($translate.instant("Success","appointmentpro"),data.message,$translate.instant("OK","appointmentpro"))})})},function(error){Loader.hide();Dialog.alert($translate.instant("Error","appointmentpro"),error.message,$translate.instant("OK","appointmentpro"))});$ionicLoading.hide()}else if(/(cancel)/.test(event.url)){browser.close();$ionicLoading.hide()}})}});browser.addEventListener('exit',function(event){deferred.reject("The sign in flow was canceled");alert('exit event='+JSON.stringify(event));$ionicLoading.hide()})}else{$window.location=googleURL}}})}
function logEvent(e){alert('LOG event='+JSON.stringify(e))}
...
$scope.readInformation=function(content){$ionicModal.fromTemplateUrl('features/appointmentpro/assets/templates/modal/booking/information.html',{scope:$scope,animation:'slide-in-right-left'}).then(function(modal){$scope.content_details=$scope.settings.cancel_policy;$scope.modalReadInformation=modal;$scope.modalReadInformation.show()})};$scope.closeModalInformation=function(){$scope.modalReadInformation.remove()}}).controller('AppointmentproGooglereturnController',function(Dialog,Appointmentpro,SB,$ionicHistory,$ionicLoading,$location,Application,$ionicPlatform,$rootScope,$ionicScrollDelegate,Loader,$timeout,$ionicSlideBoxDelegate,Customer,$scope,$filter,$state,$stateParams,$translate,$ionicModal){$scope.value_id=Appointmentpro.value_id=$stateParams.value_id;$scope.code=$stateParams.code;Loader.show();$scope.loadContent=function(){$scope.settings=Appointmentpro.getAppointmentproSettings();$scope.param={provider_id:$scope.settings.providerInfo.providerId,code:$scope.code};Appointmentpro.saveProviderGoogleToken($scope.param).then(function(data){$state.go("home",{value_id:$scope.value_id},{reload:!0}).then(function(){$state.go("appointmentpro-home",{value_id:$scope.value_id},{reload:!0}).then(function(){Loader.hide();console.log('return to google back 2',$scope.code);Dialog.alert($translate.instant("Success","appointmentpro"),data.message,$translate.instant("OK","appointmentpro"))})})},function(error){Loader.hide();Dialog.alert($translate.instant("Error","appointmentpro"),error.message,$translate.instant("OK","appointmentpro"))})}
Application.loaded.then(function(){Loader.show();$state.go("home",{value_id:$scope.value_id},{reload:!0}).then(function(){$state.go("appointmentpro-home",{value_id:$scope.value_id},{reload:!0}).then(function(){Loader.hide();Dialog.alert($translate.instant("Success","appointmentpro"),data.message,$translate.instant("OK","appointmentpro"))})})})});angular.module('starter').controller('AppointmentproClassesController',function(Dialog,Loader,Application,Customer,$session,Modal,$timeout,$translate,$ionicModal,$rootScope,Location,Appointmentpro,$scope,$state,$stateParams,$controller){angular.extend(this,$controller('AppointmentproCommanController',{Dialog:Dialog,$rootScope:$rootScope,$scope:$scope,$stateParams:$stateParams,Appointmentpro:Appointmentpro}));$scope.value_id=Appointmentpro.value_id=$stateParams.value_id;$scope.category_id=$stateParams.category_id;$scope.settings=Appointmentpro.getAppointmentproSettings();$scope.payout={};$scope.postParams={};$scope.loadContent=function(){$scope.postParams.category_id=$scope.category_id;$scope.is_loading=!0;Appointmentpro.findAllClasses($scope.postParams).success(function(data){$scope.payout=data;$scope.is_loading=!1}).error(function(error){$scope.is_loading=!1;Dialog.alert($translate.instant("Error","appointmentpro"),error.message,"OK",-1,"appointmentpro")})};$scope.getLocation=function(){if(Location.isEnabled){Location.getLocation({timeout:10000},!0).then(function(position){$scope.postParams.latitude=position.coords.latitude;$scope.postParams.longitude=position.coords.longitude;$scope.loadContent()},function(){$scope.postParams.latitude='';$scope.postParams.longitude='';$scope.requestLocation()})}else{$scope.postParams.latitude='';$scope.postParams.longitude='';$scope.requestLocation();$scope.loadContent()}}
$scope.requestLocation=function(){Location.requestLocation(function(){$scope.loadContent()},function(){$scope.loadContent()})}
