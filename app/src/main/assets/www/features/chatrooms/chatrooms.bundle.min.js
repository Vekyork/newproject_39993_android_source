...
var _buttons=[{text:$translate.instant('View user profile','chatrooms')},{text:$translate.instant('Report this message','chatrooms')},{text:$translate.instant('Block this user','chatrooms')}];if(message.senderID===Customer.customer.id){var viewProfile=_buttons[0];viewProfile.text=$translate.instant('View my profile','chatrooms');_buttons=[viewProfile]}
$scope.actionModalResolver=$ionicActionSheet.show({buttons:_buttons,cancelText:$translate.instant('Cancel','chatrooms'),cancel:function(){$scope.actionModalResolver();$scope.actionModalIsOpen=!1},buttonClicked:function(index){if(index===0){$scope.showCustomer(message.senderID)}
...
Chatrooms.find($scope.receiver_id,$scope.private_chat).success(function(data){if(angular.isObject(data)){$scope.chatroom=data.chatroom||null;$scope.recipient=data.recipient||null;$scope.room=data.room;$scope.nickname=Chatrooms.nickname=data.nickname;$scope.notifications=data.notifications;$scope.messages=data.messages;_.pullAllBy($scope.messages,Chatrooms.blockedUsers,'senderID');$scope._updateUserStatus();$scope.page_title=($scope.chatroom&&$scope.chatroom.name)||($scope.recipient&&$scope.recipient.nickname);Chatrooms.goOnline();var received_msg=function(msg){if($scope.inBlockList(msg.senderID)){return}
var ids=[];angular.forEach($scope.messages,function(m){ids.push(m.id)});if(ids.indexOf(msg.id)<0){$scope.messages.push(msg)}
...
if(angular.isString($scope.room)){Chatrooms.markRoomAsRead($scope.room)}});$scope.loadContent()});angular.module('starter').controller('ChatroomsContextualMenuController',function($scope,$rootScope,$stateParams,$controller,$q,$timeout,$translate,Chatrooms,Customer,Modal,Dialog,SB){angular.extend(this,$controller('ChatroomBaseController',{Modal:Modal,Dialog:Dialog,$q:$q,$rootScope:$rootScope,$scope:$scope,$stateParams:$stateParams,$timeout:$timeout,$translate:$translate,Chatrooms:Chatrooms,Customer:Customer,SB:SB}));angular.extend($scope,{users:{'allowed':[],'blocked':[]}});$scope.private_chat=($stateParams.type==='private');$scope.receiver_id=decodeURIComponent(atob($stateParams.receiver_id));$scope.is_loading=!0;$rootScope.$on('chatrooms.profile.updated',function(){Chatrooms.getCurrentUsers($scope.localRoom)});$rootScope.$on('chatrooms.userlist.update',function(event,payload){if($scope.localRoom===payload.room){var mappedUsers=payload.users.map(function(user){user.senderID=user.customer_id;return user});$scope.users=$scope.splitUsersByBlocked(mappedUsers)}});$scope.loadContent=function(){Chatrooms.goOnline();if($scope.is_loading){Chatrooms.find($scope.receiver_id,$scope.private_chat).success(function(data){if(angular.isObject(data)){$scope.localChatroom=data.chatroom;$scope.localRoom=data.room;Chatrooms.getCurrentUsers($scope.localRoom)}else{console.error('Expected an object, got this instead:',data)}}).error(function(error){}).finally(function(){$scope.is_loading=!1})}};$scope.loadContent()});angular.module('starter').factory('Chatrooms',function(_,$cordovaLocalNotification,$filter,$rootScope,$state,$http,$ionicPlatform,$pwaRequest,Url,$q,$window,$timeout,Application,Pages,Customer,SB,$session){var factory={value_id:null,SocketIO_Port:null,nickname:null,privateChatrooms:[],_prvchrmsKey:'private-chatrooms',watchedRooms:[],notificationSettings:[],localNotificationId:100,CONNECT_EVENT:'chatrooms:connect_event',DISCONNECT_EVENT:'chatrooms:disconnect_event',UPDATED_FRIENDLIST_EVENT:'chatrooms:updated_friendlist_event',blockedUsers:[]};factory.settings={create_private:!0,design:'list',designCard:!1};factory.reportMessage=function(messageId,reportMessage){return $pwaRequest.post('chatrooms/mobile_view/reportmessage',{urlParams:{value_id:factory.value_id},data:{messageId:messageId,reportMessage:reportMessage}})};factory.blockUser=function(senderID){var promise=$pwaRequest.post('chatrooms/mobile_report/block-user',{urlParams:{value_id:factory.value_id},data:{senderID:senderID}});promise.then(function(payload){$timeout(function(){factory.blockedUsers=payload.blocked_users});return payload});return promise};factory.unblockUser=function(senderID){var promise=$pwaRequest.post('chatrooms/mobile_report/unblock-user',{urlParams:{value_id:factory.value_id},data:{senderID:senderID}});promise.then(function(payload){$timeout(function(){factory.blockedUsers=payload.blocked_users});return payload});return promise};factory.getChatrooms=function(){if(!factory.value_id){return $pwaRequest.reject("Unknown application")}
...
$socket().emit(private_chat?'sendprivmsg':'sendmsg',payload({receiver_id:receiver,sender_avatar:Customer.getAvatarUrl(Customer.id),message:message}));return $q.resolve()};factory.sendPicture=function(receiver,image,private_chat){if(!factory.value_id||!receiver){return $q.reject("Unknown application or chatroom")}
...
if(!angular.isFunction(callback)){return $q.reject("callback must be a function")}
_callbacks[room]=_callbacks[room]||[];_callbacks[room].push(callback);return $q.resolve()};factory.markRoomAsRead=function(room){__rooms_unread[room]=[]};factory.unreadCountForRoom=function(room){__rooms_unread[room]=__rooms_unread[room]||[];return __rooms_unread[room].length};factory.handleAwayForRooms=function(rooms){if(!factory.value_id||!angular.isArray(rooms)){return $q.reject("Unknown application or rooms is not an array")}
